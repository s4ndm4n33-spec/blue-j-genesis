...
import { Code2, Copy, Play, Check, Download } from 'lucide-react';
import { useChatStream } from '@/hooks/use-chat';
import { DownloadModal } from './DownloadModal';

export function IdePanel() {
  const { selectedLanguage, myCode, setMyCode } = useBlueJStore();
  ...
  return (
    <>
      <div className="h-full flex flex-col hud-panel">
        ...
        {/* Actions */}
        <div className="p-3 border-t border-primary/20 bg-secondary/50 flex items-center justify-between gap-2">
          <button
            onClick={() => setShowDownload(true)}
            className="flex items-center gap-2 px-3 py-2 bg-accent/10 hover:bg-accent/20 border border-accent/40 text-accent rounded-sm transition-all text-xs font-hud uppercase tracking-wider"
